...
reparametrization_parameters(source::AbstractReparametrizableDistribution) = StackedVector(
...
reparametrize(source::AbstractReparametrizableDistribution, parameters::AbstractVector) = reparametrize(
    source, views(reparametrization_parameters(source), parameters)
)
...
lja_reparametrize(source::AbstractReparametrizableDistribution, target::AbstractReparametrizableDistribution, draw::AbstractVector, lja=0.) = lja_reparametrize(source, target, lpdf_and_invariants(source, draw), lja)
