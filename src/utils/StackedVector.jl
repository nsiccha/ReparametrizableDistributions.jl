struct StackedVector{E,B,V<:AbstractVector{E}} <: AbstractVector{E}
    boundaries::B
    data::V
end
...
Base.eltype(::Type{StackedVector{E}}) where {E} = E
# Base.eltype(what::StackedVector) = eltype(what.data)
Base.size(what::StackedVector) = size(what.data)
Base.IndexStyle(::Type{StackedVector{E,B,V}}) where {E,B,V} = IndexStyle(V)
...
Base.oftype(proto::StackedVector, y::AbstractVector) = StackedVector(proto.boundaries, oftype(proto.data, y))
...
stack_vector(proto::StackedVector, data::AbstractVector) = StackedVector(proto.boundaries, data)
