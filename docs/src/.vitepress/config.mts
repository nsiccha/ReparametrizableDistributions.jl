...
export default defineConfig({
  ...
  // local RD demo app) — vitepress build flags them as dead links otherwise.
  ...
  head: [
    ...
    // HTMX runtime — for inlining live RD gallery fragments via
    // `<div hx-get="…" hx-trigger="load">` placeholders.
    ['script', {src: 'https://cdn.jsdelivr.net/npm/htmx.org@2.0.8/dist/htmx.min.js'}],
    // Map HTMXObjects' --htmxo-* theme variables to VitePress's
    // brand/state tokens so embedded gallery components match the docs theme.
    ['style', {}, `
:root {
...
@media (min-width: 960px) {
  ...
  vite: {
    ...
    server: {
      ...
      host: true,
      proxy: {
        // Live RD gallery embedding (dev only). `<div hx-get="/live-rd/…">`
        // forwards to the running web app on :8096 so the docs page shows
        // live state. In production, the same path resolves to recordings
        // produced by `record!` under public/live-rd/. Override target via
        // `RD_DEV_TARGET`.
        '/live-rd': {
          target: process.env.RD_DEV_TARGET || 'http://localhost:8096',
          changeOrigin: true,
          rewrite: (path) => path.replace(/^\/live-rd/, ''),
        }
      }
    },
    optimizeDeps: {
